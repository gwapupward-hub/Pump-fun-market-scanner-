...
def classify_token(conn, mint: str) -> str | None:
    """Return a classification label for *mint*, or `None` if there is no data.

    `None` callers must skip the UPDATE — historically this function returned
    `"loser"` for unknown mints which polluted reports.
    """
    snap = fetch_one_dict(
        conn,
        """
...
def persist_classification(conn, mint: str, label: str | None) -> None:
    if label is None:
        return
